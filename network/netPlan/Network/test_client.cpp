...
int main() {
    // Server 端的监听地址
    auto msg = InitTestClient("127.0.0.0:1234");
    // Put your code Here!

    int sfd = socket(AF_INET,SOCK_STREAM,0);
    if (sfd==-1){
        myerr("Failed to create a socket");
    }
    ...
    inet_pton(AF_INET,ServerAddr,&adr);
    sv_addr.sin_addr.s_addr = adr;

    connect(sfd,(struct sockaddr*)&sv_addr,sizeof(struct sockaddr_in));
